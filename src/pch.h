...
struct QJsonUtil {
	...
	template<class T> struct JsonType;
...
template<> struct QJsonUtil::JsonType<double> {
	bool Check(JsonIt &it) { return it->isDouble(); }
	double Get(JsonIt &it) { return it->toDouble(); }
};

template<> struct QJsonUtil::JsonType<int> {
	bool Check(JsonIt &it) { return it->isDouble(); }
	int Get(JsonIt &it)
	{
		return static_cast<int>(std::round(it->toDouble()));
	}
};

template<> struct QJsonUtil::JsonType<QString> {
	bool Check(JsonIt &it) { return it->isString(); }
	QString Get(JsonIt &it) { return it->toString(); }
};

template<> struct QJsonUtil::JsonType<std::string> {
	bool Check(JsonIt &it) { return it->isString(); }
	std::string Get(JsonIt &it) { return tostdu8(it->toString()); }
};

template<> struct QJsonUtil::JsonType<bool> {
